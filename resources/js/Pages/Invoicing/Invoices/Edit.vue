...
const form = useForm({
  ...
  items: props.invoice.items.length ? props.invoice.items.map(i => ({
...
// Try to infer ppnMode
if (props.invoice.tax_amount == 0) {
  ppnMode.value = 'none'
} else {
  const sumRaw = form.items.reduce((acc, it) => acc + (it.qty * it.unit_price), 0)
  if (Math.abs(sumRaw - props.invoice.subtotal) < 2) {
    ppnMode.value = 'exclude'
  } else {
    ppnMode.value = 'include'
  }
}
...
const calculateTotal = () => {
  ...
  if (ppnMode.value === 'none') {
    ...
    form.subtotal = Math.round(rawSum / (1 + ppnRate.value / 100))
    form.tax_amount = tax
    form.total_amount = afterDisc
  }
...
const submit = () => {
  ...
  form.put(route('invoicing.invoices.update', props.invoice.id))
}
  ...
  <AuthenticatedLayout>
    <div class="max-w-4xl mx-auto space-y-6 py-6 sm:py-8 px-3 sm:px-6 lg:px-8">
      ...
      <div class="flex items-center gap-4">
        ...
        <div>
            <h2 class="text-2xl font-black text-slate-900 tracking-tight">Edit Invoice: {{ invoice.invoice_number }}</h2>
            <p class="text-slate-500 text-sm mt-1">Perbarui detail invoice yang masih berstatus draft.</p>
        </div>
      </div>
